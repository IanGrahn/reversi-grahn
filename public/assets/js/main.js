...
socket.on('join_room_response', (payload) =>{
    ...
    /* if self notify, ignore message and return */
...
socket.on('player_disconnected', (payload) =>{
    ...
    let newHTML = '<p class=\'left_room_response\'>'+payload.username+' left the Lobby (#'+payload.room+'). (There are '+payload.count+' users in this room)</p>';
    let newNode = $(newHTML);
    newNode.hide();
    $('#messages').prepend(newNode);
    newNode.show("fade",500);
